...
impl BridgedEngine {
    ...
    // pub fn new(engine: TabsEngine) -> Self {
    //     BridgedEngine {
    //         engine,
    //         incoming_payload: RefCell::default(),
    //     }
...
impl sync15_traits::BridgedEngine for BridgedEngine {
    type Error = TabsError;

    fn last_sync(&self) -> Result<i64> {
        //let new_store = TabsStore::new(self.store.storage.lock().unwrap().db_path.clone());
        let engine = TabsEngine::new(Arc::clone(&self.store));
        Ok(engine.last_sync.get().unwrap_or_default().as_millis())
        // Ok(self.engine.last_sync.get().unwrap_or_default().as_millis())
    }
    ...
    fn sync_id(&self) -> Result<Option<String>> {
        // TODO: need to potentially open this up to the store
        //let new_store = TabsStore::new(self.store.storage.lock().unwrap().db_path.clone());
        let engine = TabsEngine::new(Arc::clone(&self.store));
        let local_id = engine.local_id.borrow();
        Ok(Some(local_id.clone()))
    }

    fn reset_sync_id(&self) -> Result<String> {
        // SAM_TODO: tabs sets the local_id in prepare_for_sync and sets it to the client id
        //let engine = &TabsEngine::new(Arc::clone(&self.store));
    ...
    fn ensure_current_sync_id(&self, sync_id: &str) -> Result<String> {
        //let new_store = TabsStore::new(self.store.storage.lock().unwrap().db_path.clone());
        //let engine = &TabsEngine::new(Arc::new(new_store));
        let engine = TabsEngine::new(Arc::clone(&self.store));
        let current: Option<String> = Some(engine.local_id.borrow().clone());
        Ok(match current {
            Some(current) if current == sync_id => current,
            _ => {
                // SAM_TODO: Probably pretty hacky to just force the tabs engine to use whatever is on the server
